...
if ($args.Count -eq 1) {
    ...
    Validar-DatosEntrada -contadorVMs ($numeroMaquinas-1) -archivoEntrada $archivoEntrada -dominiosExistentes $dominiosExistentes # Se revisan los datos de entrada de todas las VMs a crear
    Clear-Host
    "Se han validado los datos para todas las VMs especificadas dentro del archivo de entrada.`nAhora se presentarán los datos de las VMs por crear para que sean validados antes de comenzar con la creación de las máquinas..."
    Start-Sleep -s 3
    # Una vez validadadas, se procede a la creación de todas las VMs
    for($i=0; $i -le $numeroMaquinas-1;$i++) { 
        Write-Host $archivoEntrada.VMs[$i].Type
        Crear-VM -contador $i -os $archivoEntrada.VMs[$i].SO
    }
} else {
